...
if args.pv_accum_dtype == 'fp32':
    kernel_int4 = qattn.qk_int4_sv_int8_accum_f32_attn # the kernel with fully fp32 accumulator
elif args.pv_accum_dtype == 'fp16+fp32':
    kernel_int4 = qattn.qk_int4_sv_int8_accum_f16_attn_inst_buf if headdim == 64 else qattn.qk_int4_sv_f16_accum_f16_attn_buf # the kernel with fp32 longterm buffer and fp16 shortterm accumulator
elif args.pv_accum_dtype == 'fp16':
    kernel_int4 = qattn.qk_int4_sv_int8_accum_f16_attn # the kernel with fully fp16 accumulator
...
for seq_len in {1024, 2048, 4096, 8192, 16384, 32768}: 
    ...
    v_fp16 = torch.randn(batch, seq_len, head, headdim, dtype=torch.float16).cuda()
    ...
    for i in range(5): kernel_int8(q, k, v, o_int8, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0,sparse)
    torch.cuda.synchronize()
    _, time_int8 = benchmark_forward(kernel_int8, q, k, v, o_int8, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0,sparse, repeats=100, verbose=False, desc='Triton')
    for i in range(5): kernel_int4(q_compact, k_compact, v, o_int4, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0,sparse)
    torch.cuda.synchronize()
    _, time_int4 = benchmark_forward(kernel_int4, q_compact, k_compact, v, o_int4, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0, sparse,repeats=100, verbose=False, desc='Triton')
    # print(o_int4)
    # print(o_int8)
    print(f'int4 {seq_len} flops:{flops/time_int4.mean*1e-12} latency:{time_int4.mean*1e3}')
    # print(f'latency:{time_int4.mean}')
    print(f'int8 {seq_len} flops:{flops/time_int8.mean*1e-12} latency:{time_int8.mean*1e3}')
...
#     o_int8 = torch.empty(batch, seq_len, head, headdim, dtype=torch.float16).cuda()
#     sm_scale = 1 / (headdim ** 0.5)
#     # for i in range(5): kernel_int8(q, k, v, o_int8, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0)
#     # torch.cuda.synchronize()
#     # _, time_int8 = benchmark_forward(kernel_int8, q, k, v, o_int8, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0, repeats=100, verbose=False, desc='Triton')
#     for i in range(5): kernel_int4(q_compact, k_compact, v, o_int4, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0, 0)
#     torch.cuda.synchronize()
#     _, time_int4 = benchmark_forward(kernel_int4, q_compact, k_compact, v, o_int4, q_scale, k_scale, 0, _is_causal, _qk_quant_gran, sm_scale, 0, 0, repeats=100, verbose=False, desc='Triton')
#     # print(o_int4)
#     # print(o_int8)
#     print(f'int4 {seq_len} flops:{flops/time_int4.mean*1e-12}')
#     # print(f'int8 {seq_len} flops:{flops/time_int8.mean*1e-12}')

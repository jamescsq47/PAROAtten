...
sparse_plan = torch.load('/home/xieruiqi/diffuser-dev/examples/cogvideo_attn/logs/calib_data/0.49_0.015_1/sparse_plan.pth', map_location = 'cuda')
...
for i in range(5): kernel_sage(q_int8, k_int8, v, kernel_out, q_scale, k_scale, 1, _is_causal, _qk_quant_gran, sm_scale, 0)
torch.cuda.synchronize()
_, time_sage = benchmark_forward(kernel_sage, q_int8, k_int8, v, kernel_out, q_scale, k_scale, 1, _is_causal, _qk_quant_gran, sm_scale, 0, repeats=100, verbose=False, desc='Triton')
print(f'Sage: latency:{time_sage.mean*1e3}, flops: {flops/time_sage.mean*1e-12}')

for i in range(5): kernel_int8(q_int8, k_int8, v, dense_kernel_out, q_scale, k_scale, 1, _is_causal, _qk_quant_gran, sm_scale, 0, all_one_sparse)
torch.cuda.synchronize()
_, time_all_dense_int8 = benchmark_forward(kernel_int8, q_int8, k_int8, v, dense_kernel_out, q_scale, k_scale, 1, _is_causal, _qk_quant_gran, sm_scale, 0, all_one_sparse, repeats=100, verbose=False, desc='Triton')
print(f'PARO: sparse ratio:1, latency:{time_all_dense_int8.mean*1e3}, flops: {flops/time_all_dense_int8.mean*1e-12}')      

for i in range(5): kernel_int8(q_int8, k_int8, v, kernel_out, q_scale, k_scale, 1, _is_causal, _qk_quant_gran, sm_scale, 0, sparse)
torch.cuda.synchronize()
_, time_int8 = benchmark_forward(kernel_int8, q_int8, k_int8, v, kernel_out, q_scale, k_scale, 1, _is_causal, _qk_quant_gran, sm_scale, 0, sparse, repeats=100, verbose=False, desc='Triton')

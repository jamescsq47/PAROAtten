...
ref_res = x + r
ref_out = rmsnorm_layer(ref_res)

ed_res, ed_out = residual_rmsnorm(r, x, rmsnorm_layer.weight)

res_all_close = torch.allclose(ref_res, ed_res, atol=1e-2, rtol=1e-4)
out_all_close = torch.allclose(ref_out, ed_out, atol=1e-2, rtol=1e-4)
...
for _ in range(WARM_UP):
    _, _ = residual_rmsnorm(r, x, rmsnorm_layer.weight)
...
print('%s %s %d %d %.4f %.4f' % (bool(res_all_close),
                                 bool(out_all_close),
                                 Z, 
                                 DIM, 

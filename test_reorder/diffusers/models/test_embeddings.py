...
def apply_rotary_emb(
    ...
    W: Optional[int] = None,
    F: Optional[int] = None
) -> Tuple[torch.Tensor, torch.Tensor]:  # 还原为元组返回类型
    """应用旋转位置编码，并支持重排序"""
    if use_real:
        cos, sin = freqs_cis
        cos = cos[None, None]
        ...
        # 如果提供了H、W、F，则进行重排序
        if mode in ["reorder", "inv_reorder"] and all(dim is not None for dim in [H, W, F]):
            # 获取序列长度
            S = cos.shape[2]
            assert S == H * W * F, f"序列长度 {S} 必须等于 H*W*F ({H}*{W}*{F}={H*W*F})"
            ...
            # 提取并重塑需要重排序的维度
            cos_seq = cos.squeeze(0).squeeze(0)  # [S, D]
            sin_seq = sin.squeeze(0).squeeze(0)  # [S, D]
            
            # 重塑为[F, H, W, D]进行重排序
            cos_seq = cos_seq.reshape(F, H, W, -1)
            sin_seq = sin_seq.reshape(F, H, W, -1)
            
            # 根据pattern进行重排序
            if pattern == "FWH":
                # [F, W, H, D]
                cos_seq = cos_seq.permute(0, 2, 1, 3)
                sin_seq = sin_seq.permute(0, 2, 1, 3)
            elif pattern == "WFH":
                # [W, F, H, D]
                cos_seq = cos_seq.permute(2, 0, 1, 3)
                sin_seq = sin_seq.permute(2, 0, 1, 3)
            elif pattern == "WHF":
                # [W, H, F, D]
                cos_seq = cos_seq.permute(2, 1, 0, 3)
                sin_seq = sin_seq.permute(2, 1, 0, 3)
            elif pattern == "HFW":
                # [H, F, W, D]
                cos_seq = cos_seq.permute(1, 0, 2, 3)
                sin_seq = sin_seq.permute(1, 0, 2, 3)
            elif pattern == "HWF":
                # [H, W, F, D]
                cos_seq = cos_seq.permute(1, 2, 0, 3)
                sin_seq = sin_seq.permute(1, 2, 0, 3)
            # FHW不需要重排序
            ...
            # 重新展平为[S, D]
            cos_seq = cos_seq.reshape(S, -1)
            sin_seq = sin_seq.reshape(S, -1)
            
            # 恢复原始形状
            cos = cos_seq.reshape(original_cos_shape)
            sin = sin_seq.reshape(original_sin_shape)

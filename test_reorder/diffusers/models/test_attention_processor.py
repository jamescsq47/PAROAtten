...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        # Apply RoPE if needed
        if image_rotary_emb is not None:
            from diffusers.models.test_embeddings import apply_rotary_emb
            query[:, :, text_seq_length:] = apply_rotary_emb(query[:, :, text_seq_length:], image_rotary_emb,H=48,W=85,F=11)
            if not attn.is_cross_attention:
                key[:, :, text_seq_length:] = apply_rotary_emb(key[:, :, text_seq_length:], image_rotary_emb,H=48,W=85,F=11)
